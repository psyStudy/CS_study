...
- 특별한 값이 없음을 나타내기위해 null 도입. → 이 값을 사용하려면 오류나도록 설계 (레코드핸들링 : 객체지향의 시초)
...
- java1.4 부터 추가
- 개발자가 “본인의 코드”에서 가정한 사실이 올바른 지 검사할 수 있게 해주는 기능. 생산자=소비자 일
...
3. **상태처럼 null의 범위를 지역(클래스, 메소드 등)에 제한하기**
