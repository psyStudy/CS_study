...
- 테이블의 특정 컬럼에 인덱스를 생성하면, 해당 컬럼 데이터를 정렬한 후 별도의 메모리 공간에 물리적 주소와 함께 저장된다.
...
- 이진트리를 혹장해 하나의 노드가 가질 수 있는 자식노드의 숫자가 2보다 큰 트리구조
- [https://code-lab1.tistory.com/217](https://code-lab1.tistory.com/217)

```sql
특징
1. 노드에는 2개 이상의 데이터(KEY)가 들어갈 수 있으며, 항상 정렬된 상태로 저장된다.
2. 내부 노드는 M/2 ~ M개의 자식을 가질 수 있다. 최대 M개의 자식을 가질 수 있는 B트리를 M차 B트리라고 한다.
3. 특정 노드의 데이터(KEY)가 K개라면 자식 노드의 개수는 K+1개 여야한다.
4. 특정 노드의 왼쪽 서브 트리는 특정 노드의 KEY보다 작은 값들로, 오른쪽 서브트리는 큰 값들로 구성된다.
5. 노드 내에 데이터는 floor(M/2)-1r개부터 최대 M-1까지 포함 될 수 있다.
```

- ex) 57 탐색
    - 맨 위 루트 노드부터 탐색이 일어나며 branch 노드를 거쳐 leaf까지 내려온다.
    - ‘57≤’를 가반으로 처음 루트 노드에서는 83
    - 아래 노드로 내려와 46,53,57 정렬된 값을 기반으로 탐색 → 57
    - 반복하여 마지막 리프 노드에 도달하여 57을 가리키는 데이터 포인터를 통해 결괏값 반환
    
    ![index_Btree.jpg](/index_Btree.jpg)
