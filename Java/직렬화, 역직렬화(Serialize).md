...
**직렬화 :** 자바 시스템 내부에서 사용되는 객체 또는 데이터를 외부의 자바 시스템에서도 사용할 수 있도록 바이트 스트림(stream of bytes)형태로 연속적인(serial)데이터로 변호나하는 포맷 변환 기술
...
휘발성 있는 캐싱 데이터의 영구 저장이 필요할 때 사용할 수 있다.
...
- 버그와 보안에 취약함.
    ...
    class Member implements Serializable {
        ...
        public Member(String name, int age) {
            if(age < 0){ // Member객체를 직려화/역직렬화 할 경우 age에 음수 값이 들어가도 걸러낼 수 없음. -> 직렬화 프록시 패턴으로 극복해야함
