...
<br></br>
## B+ 트리
B트리는 데이터 검색은 효율적이지만, 모든 데이터를 순회하는 데에는 모든 노드를 방문해야 하므로 비효율적. 이를 개선시킨 자료구조

- 오직 리프노드에만 데이터를 저장하고 리프노드가 아닌 자식노드에는 자식 포인터만 저장함 
- 리프노드끼리는 연결리스트로 연결
- 리프노드에만 데이터가 저장되기 때문에 중간 노드에서 키를 올바르게 찾기 위해 키가 중복될 수 있다
